 ...
 * answered question grouped by page and lets the tech jump back to edit
 * any section, then capture the on-site signer name and submit.
 */
export default class NeuraFormReview extends LightningElement {
    @api formObject;
    @api linkedFormId;

    // On-site signer name, carried up to the renderer on submit so it can be
    // stamped (with timestamp + geolocation) as the signature audit context.
    @track signedByName = '';

    handleSignedByChange(event) {
        this.signedByName = event?.detail?.value ?? event?.target?.value ?? '';
    }
    ...
    handleSubmit(event) {
        ...
        this.dispatchEvent(new CustomEvent('submit', {
            detail: {
                signedByName: (this.signedByName || '').trim()
            },
            bubbles: true,
            composed: true
        }));
