trigger FormSectionTrigger on Form_Section__c (after insert, after update, after delete) {
    
    Set<Id> parentIds = new Set<Id>();

    if (Trigger.isAfter) {
        if (Trigger.isInsert || Trigger.isUpdate) {
            for (Form_Section__c record : Trigger.new) {
                parentIds.add(record.Form_Template__c);
            }
        } else if (Trigger.isDelete) {
            for (Form_Section__c record : Trigger.old) {
                parentIds.add(record.Form_Template__c);
            }
        }
    }
